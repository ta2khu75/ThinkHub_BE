package com.ta2khu75.thinkhub.modules.post.api.web;
...
class PostController extends BaseController<PostApi> implements CrudController<PostRequest, PostResponse, String> {
	protected PostController(PostApi api) {
		super(api);
	}
	...
	public ResponseEntity<PostResponse> create(@Valid PostRequest post) {
		return ResponseEntity.status(HttpStatus.CREATED).body(api.create(post));
	}
	...
	public ResponseEntity<PostResponse> update(String id, @Valid PostRequest post) {
		return ResponseEntity.ok(api.update(id, post));
	}
...
//
//	public BlogController(BlogService api, ObjectMapper objectMapper, CommentService commentService) {
//		super(api);
...
//		return ResponseEntity.ok(api.search(blogSearch));
//	}
//
...
//
//		return ResponseEntity.ok(api.readAllByAuthorIdAndKeywork(authorId, keywork));
//	}
//
...
//		return ResponseEntity.status(HttpStatus.CREATED).body(api.create(blogRequest, file));
//	}
//
...
//			@RequestPart(name = "image", required = false) MultipartFile file) throws IOException {
//		BlogRequest blogRequest = objectMapper.readValue(request, BlogRequest.class);
//		return ResponseEntity.ok(api.update(id, blogRequest, file));
//	}
//
//	@DeleteMapping("/{id}")
