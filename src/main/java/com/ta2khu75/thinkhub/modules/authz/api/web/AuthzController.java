package com.ta2khu75.thinkhub.modules.authz.api.web;
...
public class AuthzController extends BaseController<AuthzApi> {
	protected AuthzController(AuthzApi api) {
		super(api);
	}
	...
	public ResponseEntity<List<RoleResponse>> readAllRoles() {
		return ResponseEntity.ok(api.readAllRoles());
	}
	...
	public ResponseEntity<RoleResponse> create(@Valid @RequestBody RoleRequest request) {
		return ResponseEntity.status(HttpStatus.CREATED).body(api.createRole(request));
	}
	...
	public ResponseEntity<RoleResponse> update(@PathVariable Long id, @Valid @RequestBody RoleRequest request) {
		return ResponseEntity.ok(api.updateRole(id, request));
	}
	...
	public ResponseEntity<RoleResponse> read(@PathVariable Long id) {
		return ResponseEntity.ok(api.readRole(id));
	}
	...
	public ResponseEntity<List<PermissionGroupResponse>> readAllGroups() {
		return ResponseEntity.ok(api.readAllGroups());
	}
}
