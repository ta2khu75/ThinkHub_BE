package com.ta2khu75.thinkhub.modules.result.api.controller;
...
@Tag(name = "Quiz Attempt", description = "Manage quiz attempt lifecycle")
...
public class QuizAttemptController implements IdDecodable {
	...
	public ResponseEntity<QuizResultResponse> start(@PathVariable String id) {
		QuizResultResponse response = quizResultService.readByQuizId(id);
		if (response != null) {
			return ResponseEntity.ok(response);
		}
		return ResponseEntity.status(HttpStatus.CREATED).body(quizResultService.take(id));
	}
