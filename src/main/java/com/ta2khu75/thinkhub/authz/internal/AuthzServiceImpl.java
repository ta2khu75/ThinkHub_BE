...
public class AuthzServiceImpl implements AuthzApi {
	...
	@Override
	public RoleDto readRoleDto(Long id) {
		return roleService.readDto(id);
	}

	@Override
	public RoleDto readRoleDtoByName(String name) {
		return roleService.readDtoByName(name);
	}
	...
	private PermissionResponse createOrUpdatePermission(PermissionRequest request) {
		try {
			PermissionResponse permission = permissionService.readByPatternAndMethod(request.pattern(),
					request.method());
			return permissionService.update(permission.id(), request);
		} catch (Exception e) {
