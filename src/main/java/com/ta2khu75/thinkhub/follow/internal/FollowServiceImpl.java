...
public class FollowServiceImpl implements FollowApi {
	...
	@Override
	public PageResponse<AuthorResponse> readPage(Long followingId, FollowDirection direction, Search search) {
		Pageable pageable = search.toPageable();
		Page<Follow> page;
		PageResponse<FollowResponse> pageResponse;
		if (direction == FollowDirection.FOLLOWING) {
			page = repository.findByIdFollowingId(followingId, pageable);
			...
			page = repository.findByIdFollowerId(followingId, pageable);
		...
		return mapToAuthorResponse(pageResponse);
	}
	...
	private PageResponse<AuthorResponse> mapToAuthorResponse(PageResponse<FollowResponse> pageResponse) {
		...
		List<AuthorResponse> authors = new ArrayList<>(accountApi.readMapAuthorsByAccountIds(accountIds).values());
